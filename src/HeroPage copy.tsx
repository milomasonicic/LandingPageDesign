  ...
  return (
    ...
    <div className="h-screen flex items-center justify-center sticky top-0">
        {texts.map((text, index) => (
          <motion.div
            key={index}
            initial={{ opacity: 0, y: 100 }} // Početna pozicija je 100px ispod ekrana
            style={{ color: color }}
            animate={{
              opacity: index === currentIndex ? 1 : 0, // Tekst postaje vidljiv kad se odgovarajući index prikaže
              y: index === currentIndex ? 0 : 100, // Tekst dolazi od dole i postepeno penje
            }}
            transition={{
              opacity: { duration: 0.5 },
              y: { type: "spring", stiffness: 100, damping: 25 }, // Prolećni efekat za y
            }}
