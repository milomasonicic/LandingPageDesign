...
function Land() {
    ...
    return(
        <main className="mx-auto max-w-[1440px] relative mt-12">
             ...
             <section className="flex h-[570px] md:h-[730px]  flex-col justify-center items-center relative">
            ...
            <section className="flex h-[570px] md:h-[730px] flex-col justify-center items-center">
...
function Land() {
  ...
  // Funkcija za promenu sekcija pomoću skrola
  const handleScroll = useCallback((event) => {
    if (event.deltaY > 0) {
      // Skrol nadole -> sledeća sekcija
      setCurrent((prev) => (prev + 1) % sections.length);
    } else if (event.deltaY < 0) {
      // Skrol nagore -> prethodna sekcija
  ...
  return (
    <main className="mx-auto max-w-[1440px] relative mt-12 h-[700px] overflow-hidden">
      <AnimatePresence>
        <motion.section
          key={current}
          className="flex h-full flex-col justify-center items-center absolute top-0 left-0 w-full"
